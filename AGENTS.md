...
Agent activity (`git-activity agents`) is a sibling pipeline: detect local tools → filter by `~/.git-activity/agents.json` → each adapter lists sessions → group by project cwd → merge overlapping intervals → TUI / export. All seven sources parse sessions: Claude Code, Pi, Codex, GitHub Copilot CLI, VS Code Copilot Chat, Cursor, and OpenCode. Each adapter lives in its own file under `src/agents/sources/` with a fixture test, reads local session stores without loading full transcripts, and reports a broken file as a warning so one bad source never fails the report. Shared parsing helpers live in `sources/common.ts`. OpenCode reads `~/.local/share/opencode/opencode.db` (SQLite) rather than JSONL.
